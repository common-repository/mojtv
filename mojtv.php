    ...
    class mojtv extends WP_Widget {
        public function __construct()
        {
        	add_action('admin_init', array(&$this, 'admin_init'));
					...
					parent::__construct( false , __('Moj TV', 'wp_widget_plugin'));
        }
        ...
        public static function deactivate()
        {
            unlink(MOJTV_PLUGIN_CACHE_DIR.'/channels.xml');
				...
				public function init_settings()
				{
					...
					register_setting('mojtv-group', 'mojtv_setting_target');
					register_setting('mojtv-group', 'mojtv_setting_refresh');
				}
				...
				function widget($args, $instance) {
						extract( $args );
						$fileTimeAgo = time()-filemtime(MOJTV_PLUGIN_CACHE_DIR.'/movies.xml');
						$fileTimeAgo = ceil ($fileTimeAgo/60);
						$minutaTxt = '';
						...
						if($fileTimeAgo < 5) {
						 ...
						 echo moj_widget();
						 echo '</ul>'. (get_option('mojtv_setting_refresh') == '2' ? '<span class="refreshedAgo">Sadržaj osvježen prije '. $fileTimeAgo .' '. $minutaTxt .'</span>' : '');
						 echo $after_widget;
				}
    }
}
    ...
    register_deactivation_hook(__FILE__, array('mojtv', 'deactivate'));
		...
		function mojtv_widg() {
			...
			if(get_option('mojtv_setting_type') == '0') {
				...
				$fileTimeAgo = time()-filemtime(MOJTV_PLUGIN_CACHE_DIR.'/series.xml');
			}
			...
			if(get_option('mojtv_setting_height') != '') {
				return '<ul class="movielist" style="height: '. intval(get_option('mojtv_setting_height')) .'px;">'.moj_widget().'</ul>'. (get_option('mojtv_setting_refresh') == '2' ? '<span class="refreshedAgo">Sadržaj osvježen prije '. $fileTimeAgo .' '. $minutaTxt .'</span>' : '');
			} else {
				return '<ul class="movielist">'.moj_widget().'</ul>'. (get_option('mojtv_setting_refresh') == '2' ? '<span class="refreshedAgo">Sadržaj osvježen prije '. $fileTimeAgo .' '. $minutaTxt .'</span>' : '');
			}
		}
